---
title: ""
format:
  html:
    toc: true
    toc-title: On this page
    number-sections: false
    anchor-sections: true
    page-layout: full
    html-math-method: mathjax
    pandoc-args: ["--number-offset=0"]
---

## Research{#research-vision}
I develop safe, interpretable, and adaptive AI systems for real-world cyber-physical environments that must operate under uncertainty, strict physical constraints, and adversarial conditions. My work sits at the intersection of machine learning, control theory, optimization, and AI safety, with a core focus on building models that are robust, predictable, and deployable at scale.

<div style="
  display:flex;
  align-items:center;
  justify-content:space-between;
  margin-bottom:0.6rem;
">
  <h3 style="margin:0; color:#7a2e1f;">Vision</h3>

  <a href="https://kundan-kumarr.github.io/publications/"
     style="
       display:inline-flex;
       align-items:center;
       gap:6px;
       padding:6px 14px;
       border:1.5px solid #7a2e1f;
       border-radius:999px;
       font-size:0.85rem;
       font-weight:600;
       color:#7a2e1f;
       background:rgba(122,46,31,0.06);
       text-decoration:none;
       white-space:nowrap;
     ">
    Full Publications →
  </a>
</div>


I aim to create AI agents that generalize across environments, learn from imperfect or unreliable data, and remain stable under distribution shifts. These capabilities are essential both for scientific progress and for building production-grade intelligent systems used in industry.

My research centers around five pillars:
  
- Safe & Trustworthy Reinforcement Learning: Designing agents that remain reliable under sensor noise, hardware faults, non-stationarity, and adversarial perturbations. This includes constraint-aware learning, certified robustness, and safe exploration in safety-critical settings.
- Physics-informed Deep Reinforcement Learning (DRL): Embedding physical laws, invariants, and feasibility constraints directly into model architectures and learning objectives to improve convergence, interpretability, and real-world deployability.
- Probabilistic & Bayesian Modeling: Quantifying epistemic and aleatoric uncertainty for risk-aware planning in partially observable, high-stakes environments using Bayesian neural networks, uncertainty-aware RL, and probabilistic inference.
- Large Language Models (LLMs) for autonomous reasoning: Leveraging LLMs to support high-level planning, explainable decision pipelines, natural-language supervision, and adaptive control. This enables human-AI collaboration and interpretable reasoning in complex systems.
-  Vision-based simulation environments: Using platforms such as CARLA, CityLearn, AirSim, and OpenDSS to train agents in multimodal, visually rich environments, supporting robust perception-control integration and sim-to-real transfer.
  

### Mission
By integrating physics-guided structure, probabilistic reasoning, and safe reinforcement learning, my mission is to build the next generation of AI systems that are:

- Reliable, even under uncertainty, noise, and adversarial conditions
- Generalizable across tasks, scales, and distribution shifts 
- Interpretable to operators, engineers, and decision-makers
- Deployable in large-scale, real-world cyber-physical environments

My research advances the foundation needed to deploy trustworthy AI across critical infrastructures, autonomous systems, and safety-critical infrastructure domains where reliability and robustness are essential for real-world impact.

## Research Focus {#research-focus}
These focus areas organize my ongoing and recent projects that bridge **fundamental methods** and **deployable systems**.
<!-- Styling (optional if handled globally) -->

```{=html}
<style>
  .proj-grid {
    display: grid;
    grid-template-columns: repeat(auto-fill, minmax(300px, 1fr));
    gap: 1.5rem;
    margin-top: 0.5rem;
  }

  .proj-card {
    border: 2px solid #ddd;
    border-radius: 12px;
    padding: 1.2rem;
    background: #fff;
    box-shadow: 0 1px 6px rgba(0, 0, 0, 0.05);
    display: flex;
    flex-direction: column;
    align-items: center;
    transition: transform 0.2s ease;
    max-width: 360px;
    margin: 0 auto;
    text-align: center;

    /* ✅ kill hyperlink look inside the whole card */
    text-decoration: none !important;
    color: inherit !important;
  }

  /* ✅ also kill underline/color on any nested elements */
  .proj-card * {
    text-decoration: none !important;
    color: inherit;
  }

  .proj-card:hover {
    transform: translateY(-3px);
    text-decoration: none !important;
  }

  /* Small, centered hex-badge style */
  .proj-card img {
    width: 130px;
    height: 130px;
    object-fit: contain;
    display: block;
    margin: 0.4rem auto 0.9rem auto;
    background-color: transparent;
    padding: 0;
    border-bottom: none;
  }

  .proj-body {
    font-family: 'Nunito', system-ui, -apple-system, BlinkMacSystemFont, sans-serif;
  }

  .group-label {
    font-size: 1.2rem;
    font-weight: 600;
    margin: 2rem 0 0.5rem 0;
    color: #6c2b88;
    display: flex;
    align-items: center;
    justify-content: center;
    gap: 0.5rem;
    text-align: center;
  }

  .proj-llm strong {
    color: #8B008B !important;
    font-size: 1.05rem;
  }

  /* Optional: make body text slightly softer */
  .proj-body {
    color: #4c4c4c;
  }
</style>

<div class="research-groups">

  <a class="research-group-card" href="research/smart-energy.qmd">
    <div class="rg-content">
      <strong>Safe & Intelligent Systems for Smart Energy</strong>
      <p>Physics-informed deep RL, safe and uncertainty-aware control, LLM-guided planning, and sim-to-real transfer for smart grid and DER-integrated energy systems.</p>
      <span class="rg-tags">
        <span>Volt-VAR Control</span>
        <span>Physics-Informed RL</span>
        <span>Uncertainty Quantification</span>
        <span>LLM Control</span>
        <span>Domain Adaptation</span>
      </span>
    </div>
  </a>

  <a class="research-group-card" href="research/ai-safety.qmd">
    <div class="rg-content">
      <strong>AI Safety & Alignment</strong>
      <p>Behavioral and mechanistic evaluations for deceptive alignment, adversarial robustness, reward hacking detection, and scalable oversight for agentic LLM systems.</p>
      <span class="rg-tags">
        <span>Evaluation Awareness</span>
        <span>Red-Teaming</span>
        <span>Scalable Oversight</span>
        <span>Monitor Transferability</span>
      </span>
    </div>
  </a>

  <a class="research-group-card" href="research/vision-simulation.qmd">
    <div class="rg-content">
      <strong>Vision & Simulation for Autonomous Systems</strong>
      <p>End-to-end perception-control pipelines, multimodal sensor fusion, and sim-to-real transfer using CARLA, AirSim, and OpenDSS for autonomous and cyber-physical systems.</p>
      <span class="rg-tags">
        <span>CARLA</span>
        <span>Perception-Control</span>
        <span>Multimodal Fusion</span>
        <span>Sim-to-Real</span>
      </span>
    </div>
  </a>

</div>

<style>
.research-groups {
  display: grid;
  grid-template-columns: repeat(3, 1fr);
  gap: 1.2rem;
  margin-top: 1.5rem;
}

.research-group-card {
  display: flex;
  flex-direction: column;
  padding: 1.4rem 1.6rem;
  border: 1.5px solid #e0d6cc;
  border-radius: 12px;
  background: #fff;
  text-decoration: none !important;
  color: inherit !important;
  transition: box-shadow 0.2s ease, transform 0.15s ease;
  border-top: 4px solid #b46939;
}

.research-group-card:hover {
  box-shadow: 0 4px 16px rgba(0,0,0,0.08);
  transform: translateY(-3px);
  text-decoration: none !important;
}

.rg-content strong {
  font-size: 1.05rem;
  font-weight: 700;
  color: #7a2e1f;
  display: block;
  margin-bottom: 0.5rem;
}

.rg-content p {
  font-size: 0.88rem;
  color: #555;
  line-height: 1.6;
  margin: 0 0 0.8rem 0;
}

.rg-tags {
  display: flex;
  flex-wrap: wrap;
  gap: 0.4rem;
  margin-top: auto;
}

.rg-tags span {
  font-size: 0.72rem;
  background: #f5f0e9;
  color: #7a2e1f;
  padding: 2px 10px;
  border-radius: 999px;
  border: 1px solid #e0d0c0;
}

@media (max-width: 768px) {
  .research-groups {
    grid-template-columns: 1fr;
  }
}
</style>
<!-- Group: DRL-based Control -->

<div class="group-label" style="justify-content:flex-start;">DRL-based Control</div>

<div class="proj-grid">

  <a class="proj-card" href="drl_vvc/index.qmd">
    <span class="proj-body proj-llm">
      <img src="drl_vvc/drl_vvc.png" alt="DRL Control">
      <strong>DRL for Volt-VAR</strong><br>
      Design control agents for voltage regulation and reactive power optimization in smart distribution grids.
    </span>
  </a>

  <a class="proj-card" href="physics-informed-actor-critic/index.qmd">
    <span class="proj-body proj-llm">
      <img src="physics-informed-actor-critic/physics.png" alt="Actor Critic">
      <strong>Physics-Informed Actor-Critic</strong><br>
      Embed grid physics and control limits directly into the DRL learning loop for stable and efficient decisions.
    </span>
  </a>

  <a class="proj-card" href="sim2real/index.qmd">
    <span class="proj-body proj-llm">
      <img src="sim2real/sim2real.png" alt="Sim2Real Transfer">
      <strong>Sim-to-Real Transfer</strong><br>
      Train agents in simulated OpenDSS environments and deploy them on real-time OPAL-RT setups.
    </span>
  </a>

</div>

<!-- Group: 🔐 Safe & Trustworthy RL -->

<div class="group-label" style="justify-content:flex-start;">Safe & Trustworthy RL</div>

<div class="proj-grid">

  <a class="proj-card" href="safe/index.qmd">
    <span class="proj-body proj-llm">
      <img src="safe/safe.png" alt="Safe RL">
      <strong>Robust & Stable Learning</strong><br>
      Develop agents that ensure system safety, robustness, and interpretability under uncertainty.
    </span>
  </a>

  <a class="proj-card" href="uncertainty/index.qmd">
    <span class="proj-body proj-llm">
      <img src="uncertainty/uncertainty.png" alt="Uncertainty Quantification">
      <strong>Uncertainty-Aware Policies</strong><br>
      Quantify epistemic and aleatoric uncertainty in high-stakes, partially observable settings.
    </span>
  </a>

</div>

<!-- Group: Transfer & Meta-Adaptation -->

<div class="group-label" style="justify-content:flex-start;">Transfer & Meta-Adaptation</div>
<div class="proj-grid">

  <a class="proj-card" href="domain_adaptation/index.qmd">
    <span class="proj-body proj-llm">
      <img src="domain_adaptation/domain_adaptation.png" alt="Transfer Learning">
      <strong>Domain Adaptation</strong><br>
      Enable agents to generalize across grids with different topologies, dynamics, and loads.
    </span>
  </a>

  <a class="proj-card" href="meta_rl/index.qmd">
    <span class="proj-body proj-llm">
      <img src="meta_rl/meta_rl.png" alt="Meta RL">
      <strong>Meta-RL for Efficiency</strong><br>
      Leverage meta-reasoning to accelerate learning in low-data, high-variance scenarios.
    </span>
  </a>

</div>

<!-- Group: Vision-Simulation Integration -->

<div class="group-label" style="justify-content:flex-start;">Vision-Simulation Integration</div>
<div class="proj-grid">

  <a class="proj-card" href="carla/index.qmd">
    <span class="proj-body proj-llm">
      <img src="carla/carla.png" alt="Perception Control">
      <strong>Perception-Control Fusion</strong><br>
      Use CARLA and AirSim to train end-to-end systems in visual RL tasks with sensors.
    </span>
  </a>

  <a class="proj-card" href="multimodal/index.qmd">
    <span class="proj-body proj-llm">
      <img src="multimodal/multimodal.png" alt="Multimodal Fusion">
      <strong>Multi-modal Representations</strong><br>
      Combine visual, state, and contextual features for better decision-making.
    </span>
  </a>

</div>

<!-- Group: LLM-Augmented Decision Systems -->

<div class="group-label" style="justify-content:flex-start;">LLM-Augmented Decision Systems</div>

<div class="proj-grid">

  <a class="proj-card" href="llm_control/index.qmd">
    <span class="proj-body proj-llm">
      <img src="llm_control/llm_control.png" alt="LLM-guided control">
      <strong>LLM-Guided Control</strong><br>
      Translate natural language into actionable policies for real-world environments.
    </span>
  </a>

</div>

<div class="group-label" style="justify-content:flex-start;">AI Safety & Alignment</div>

<div class="proj-grid">

  <a class="proj-card" href="#">
    <span class="proj-body proj-llm">
      <img src="safe/safe.png" alt="Eval Awareness">
      <strong>Evaluation Awareness</strong><br>
      Detecting when models behave differently under evaluation vs. deployment — deceptive alignment and monitor transferability.
    </span>
  </a>

  <a class="proj-card" href="#">
    <span class="proj-body proj-llm">
      <img src="safe/safe.png" alt="Adversarial Robustness">
      <strong>Adversarial Robustness & Red-Teaming</strong><br>
      Systematic stress-testing of LLM agents for reward hacking, obfuscated internal states, and behavioral failures.
    </span>
  </a>

  <a class="proj-card" href="#">
    <span class="proj-body proj-llm">
      <img src="uncertainty/uncertainty.png" alt="Scalable Oversight">
      <strong>Scalable Oversight</strong><br>
      Designing oversight mechanisms and safety benchmarks for agentic systems operating in high-stakes environments.
    </span>
  </a>

</div>
```


## Publications {#publications}

::: {.panel-tabset}

### Journal Papers
```{=html}
<h3 style="
  display:flex;
  justify-content:space-between;
  align-items:center;
  margin-bottom:.8rem;
">
  <span>Journal Papers</span>

  <span style="
    padding:4px 14px;                 /* bigger */
    border:1.5px solid #cfd8e3;       /* softer border */
    border-radius:999px;
    font-size:0.95rem;                /* slightly larger text */
    color:#1f3a5f;                    /* calm professional blue */
    background:#f5f9ff;               /* subtle background */
    margin-left:auto;                 /* pushes fully right */
  ">
    Total: <strong style="margin-left:4px;">2</strong>
  </span>
</h3>

<style>
  .paper-title {
    color: #0077cc;        /* professional blue for titles */
    font-weight: 600;
  }
  .author-me {
    color: #228B22;        /* same blue for your name */
    font-weight: 600;
  }
</style>

<ol reversed>
<li>
  Arif Hussian, <span class="author-me">Kundan Kumar</span>, Gelli Ravikumar<br>

  <span>
    Bayesian-optimized bidirectional long-short-term memory network for wind power forecasting with uncertainty quantification
  </span>,
  <em>Electric Power Systems Research</em>, 2026<br>

  <span class="paper-actions">
    <a class="pub-btn pub-paper"
       href="https://www.sciencedirect.com/science/article/pii/S037877962500848X"
       target="_blank" rel="noopener">
      Paper
    </a>

    <a class="pub-btn pub-code"
       href="#"
       target="_blank" rel="noopener">
      Code
    </a>

    <a class="pub-btn pub-poster"
       href="#"
       target="_blank" rel="noopener">
      ️Poster
    </a>
  </span>
</li>


<li>
  <span class="author-me">Kundan Kumar</span>, Gelli Ravikumar<br>

  <span>
    Physics-based Deep Reinforcement Learning for Grid-Resilient Volt-VAR Control
  </span> (Under Review),
  <em>IEEE Transactions on Smart Grid</em>, 2025<br>

  <span class="paper-actions">
    <a class="pub-btn pub-paper"
       href=""
       target="_blank" rel="noopener">
      Paper
    </a>

    <a class="pub-btn pub-code"
       href="#"
       target="_blank" rel="noopener">
      Code
    </a>

    <a class="pub-btn pub-poster"
       href="#"
       target="_blank" rel="noopener">
       Poster
    </a>
  </span>
</li>


</ol>
<!-- toggle button -->
<div style="text-align:center;margin-top:.5rem;">
  <button onclick="toggleConfPapers()" style="
    padding:6px 14px;
    font-size:.85rem;
    background:#f1f1f1;
    color:#333;
    border:1px solid #aaa;
    border-radius:6px;
    cursor:pointer;">
    Show More
  </button>
</div>

<script>
function toggleConfPapers(){
  const section=document.getElementById('conf-more');
  const btn=event.target;
  if(section.style.display==='none'){
    section.style.display='block';
    btn.textContent='Show Less';
  }else{
    section.style.display='none';
    btn.textContent='Show More';
  }
}
</script>

```

### Conference Papers

```{=html}
<h3 style="
  display:flex;
  justify-content:space-between;
  align-items:center;
  margin-bottom:.8rem;
">
  <span>Conference Papers</span>

  <span style="
    padding:4px 14px;                 /* bigger */
    border:1.5px solid #cfd8e3;       /* softer border */
    border-radius:999px;
    font-size:0.95rem;                /* slightly larger text */
    color:#1f3a5f;                    /* calm professional blue */
    background:#f5f9ff;               /* subtle background */
    margin-left:auto;                 /* pushes fully right */
  ">
    Total: <strong style="margin-left:4px;">7</strong>
  </span>
</h3>

<style>
  .paper-actions{
    display:inline-flex;
    gap:8px;
    flex-wrap:wrap;
    margin-top:6px;
  }

  .pub-btn{
    display:inline-flex;
    align-items:center;
    gap:6px;
    padding:6px 12px;
    border-radius:10px;
    border:1.8px solid currentColor;
    text-decoration:none;
    font-size:.86rem;
    font-weight:600;
    line-height:1;
    background:transparent;
    transition: color .15s ease, background-color .15s ease, box-shadow .15s ease;
  }

  .pub-btn:hover{
    box-shadow: 0 2px 6px rgba(0,0,0,0.08);
  }

  /* ===== Button types ===== */

  .pub-paper{ color:#1f5fbf; }
  .pub-paper:hover{
    color:#174a94;                      /* darker blue */
    background: rgba(31,95,191,0.08);
  }

  .pub-code{ color:#1f8f4d; }
  .pub-code:hover{
    color:#166b39;                      /* darker green */
    background: rgba(31,143,77,0.08);
  }

  .pub-pres{ color:#d93025; }
  .pub-pres:hover{
    color:#a61f18;                      /* darker red */
    background: rgba(217,48,37,0.08);
  }

  .pub-poster{ color:#7b4fd6; }
  .pub-poster:hover{
    color:#5f3fb0;                      /* darker purple */
    background: rgba(123,79,214,0.08);
  }
</style>


<ol reversed>
  <li>
    <span class="author-me">Kundan Kumar</span>, Gelli Ravikumar<br>

    <span>
      A Multi-Objective Optimization Framework for Carbon-Aware Smart Energy Management
    </span>,
    IEEE North American Power Symposium (NAPS), 2025<br>

    <span class="paper-actions">
      <a class="pub-btn pub-paper"
         href="YOUR_PAPER_LINK"
         target="_blank" rel="noopener">
        Paper
      </a>

      <a class="pub-btn pub-pres"
         href="YOUR_SLIDES_LINK"
         target="_blank" rel="noopener">
        Presentation
      </a>
    </span>
  </li>

<li>
  <span class="author-me">Kundan Kumar</span>, Kumar Utkarsh, Wang Jiyu, Padullaparti Harsha<br>

  <span>
    Advanced Semi-Supervised Learning With Uncertainty Estimation for Phase Identification in Distribution Systems
  </span>,
  IEEE PES General Meeting, 2025<br>

  <span class="paper-actions">
    <a class="pub-btn pub-paper"
       href="YOUR_PAPER_LINK"
       target="_blank" rel="noopener">
      Paper
    </a>

    <a class="pub-btn pub-pres"
       href="/misc/info/presentations/SSL_PESGM_presentation.pdf"
       target="_blank" rel="noopener">
      Presentation
    </a>

    <a class="pub-btn pub-poster"
       href="/misc/info/posters/SSL_PESGM_poster.pdf"
       target="_blank" rel="noopener">
       Poster
    </a>
  </span>
</li>

<li>
  <span class="author-me">Kundan Kumar</span>, Gelli Ravikumar<br>

  <span>
    Transfer Learning Enhanced Deep Reinforcement Learning for Volt-Var Control in Smart Grids
  </span>,
  IEEE PES Grid Edge Technologies Conference & Exposition, 2025<br>

  <span class="paper-actions">
    <a class="pub-btn pub-paper"
       href="https://arxiv.org/abs/2202.13541"
       target="_blank" rel="noopener">
      Paper
    </a>

    <a class="pub-btn pub-poster"
       href="#"
       target="_blank" rel="noopener">
       Poster
    </a>
  </span>
</li>


  <!-- start collapsible -->
  <div id="conf-more" style="display:none;">

<li>
  <span class="author-me">Kundan Kumar</span>, Aditya Akilesh Mantha, Gelli Ravikumar<br>

  <span>
    Bayesian Optimization for Deep Reinforcement Learning in Robust Volt-Var Control
  </span>,
  IEEE PES General Meeting, 2024<br>

  <span class="paper-actions">
    <a class="pub-btn pub-paper"
       href="https://arxiv.org/abs/2202.13541"
       target="_blank" rel="noopener">
      Paper
    </a>

    <a class="pub-btn pub-poster"
       href="#"
       target="_blank" rel="noopener">
       Poster
    </a>
  </span>
</li>

 <li>
  <span class="author-me">Kundan Kumar</span>, Gelli Ravikumar<br>

  <span>
    Deep RL-based Volt-VAR Control and Attack Resiliency for DER-Integrated Distribution Grids
  </span>,
  IEEE Innovative Smart Grid Technologies (ISGT), 2024<br>

  <span class="paper-actions">
    <a class="pub-btn pub-paper"
       href="https://ieeexplore.ieee.org/document/10454163"
       target="_blank" rel="noopener">
      Paper
    </a>

    <a class="pub-btn pub-poster"
       href="#"
       target="_blank" rel="noopener">
       Poster
    </a>
  </span>
</li>


<li>
  JK Francis, C Kumar, J Herrera-Gerena, <span class="author-me">Kundan Kumar</span>, MJ Darr<br>

  <span>
    Deep Learning and Pattern-based Methodology for Multivariable Sensor Data Regression
  </span>,
  IEEE ICMLA, 2022<br>

  <span class="paper-actions">
    <a class="pub-btn pub-paper"
       href="https://ieeexplore.ieee.org/document/10069730"
       target="_blank" rel="noopener">
      Paper
    </a>
  </span>
</li>


<li>
  Kin Gwn Lore, Nicholas Sweet, <span class="author-me">Kundan Kumar</span>, N Ahmed, S Sarkar<br>

  <span>
    Deep Value of Information Estimators for Collaborative Human-Machine Information Gathering
  </span>,
  ACM/IEEE ICCPS, 2016<br>

  <span class="paper-actions">
    <a class="pub-btn pub-paper"
       href="https://arxiv.org/abs/1512.07592"
       target="_blank" rel="noopener">
      Paper
    </a>
  </span>
</li>


  </div>
</ol>

<!-- toggle button -->
<div style="text-align:center;margin-top:.5rem;">
  <button onclick="toggleConfPapers()" style="
    padding:6px 14px;
    font-size:.85rem;
    background:#f1f1f1;
    color:#333;
    border:1px solid #aaa;
    border-radius:6px;
    cursor:pointer;">
    Show More
  </button>
</div>

<script>
function toggleConfPapers(){
  const section=document.getElementById('conf-more');
  const btn=event.target;
  if(section.style.display==='none'){
    section.style.display='block';
    btn.textContent='Show Less';
  }else{
    section.style.display='none';
    btn.textContent='Show More';
  }
}
</script>
```

:::

## Ongoing Projects {#ongoing-projects}

- Federated DRL for Cyber-Resilient Volt-VAR Optimization  
  Decentralized, communication-efficient control using LSTM-enhanced PPO agents across distributed DERs.

- One-Shot Policy Transfer with Physics Priors  
  Train agents on small topologies and adapt to IEEE 123-bus, 8500-node networks in a few iterations.

- LLM-Guided Autonomous Planning for Smart Buildings  
  Convert user prompts to interpretable control policies using LLMs (OpenAI, Claude) in CityLearn environments.

